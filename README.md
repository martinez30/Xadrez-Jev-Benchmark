...
### Play against Jev locally

The Jev opponent uses the [TypeSafe System One API](https://api.typesafe.ai/docs).
The API key stays in a local server process; it is never sent to the browser or
included in the static site. Get an API key from TypeSafe, then in PowerShell:

```powershell
$env:JEV_API_KEY = "your-key-here"
dotnet publish Engine.UI -c Release
dotnet run --project Engine.Jev.Host -c Release
```

Open <http://127.0.0.1:5208/>, choose **New game**, then **Jev**. The local host
serves the published interface and forwards Jev decisions. It listens only on
`127.0.0.1`. You can set `JEV_MODEL` to a model name available to your TypeSafe
account; the default is `jev-latest`. If your SDK reports `NETSDK1226`, append
`-p:AllowMissingPrunePackageData=true` to the .NET commands.

Jev is an experimental opponent with no Elo rating. Every response is checked
against moves generated by the engine. API errors pause its turn and show a retry
button. The public static site remains playable against the original bots, but
cannot call Jev without the local host.
